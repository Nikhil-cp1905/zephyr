...
#include <stdio.h>
#include <stdlib.h>
#include "../include/shihai/lib/drive.h"   // Include the header file for drive-related functions
#include "../include/shihai/lib/sbus.h"    // Include the header file for SBUS-related functions
...
void simulate_sbus_signal(uint8_t *buffer) {
    ...
    for (int i = 0; i < SBUS_CHANNELS; i++) {
        uint32_t pwm_value = sbus_pwm_interpolation(channels[i], (uint32_t[]){1000, 2000, 2000}, (uint16_t[]){0, 2047});
